document.addEventListener('DOMContentLoaded', () => {
    ...
    function initSkillCalculator() {
        ...
        const performCalculation = () => {
            ...
            const totalCost =
                cumulativeCosts[rarity][desiredPoints - 1] -
                (startPoints > 0 ? cumulativeCosts[rarity][startPoints - 1] : 0);
    ...
    function initExpCalculator() {
        ...
        const expCosts = {
            legendary: [
                120, 360, 720, 1200, 3600, 7200, 10800, 14400, 18000, 22200, 27000, 32400, 38400, 45000, 52200, 60000,
                67800, 75600, 84000, 90000, 96000, 103200, 110400, 117600, 126000, 134400, 142800, 151200, 162000,
                180000, 204000, 234000, 270000, 312000, 360000, 414000, 474000, 540000, 660000, 810000, 960000, 1140000,
                1320000, 1530000, 1740000, 1980000, 2220000, 2520000, 2820000, 2820000, 2820000, 2820000, 2820000,
                2820000, 2820000, 2820000, 2820000, 2820000, 2820000,
            ],
            epic: [
                100, 300, 600, 1000, 3000, 6000, 9000, 12000, 15000, 18500, 22500, 27000, 32000, 37500, 43500, 50000,
                56500, 63000, 70000, 75000, 80000, 86000, 92000, 98000, 105000, 112000, 119000, 126000, 135000, 150000,
                170000, 195000, 225000, 260000, 300000, 345000, 395000, 450000, 550000, 675000, 800000, 950000, 1100000,
                1275000, 1450000, 1650000, 1850000, 2100000, 2350000, 2350000, 2350000, 2350000, 2350000, 2350000,
                2350000, 2350000, 2350000, 2350000, 2350000,
            ],
            elite: [
                80, 240, 480, 800, 2400, 4800, 7200, 9600, 12000, 14800, 18000, 21600, 25600, 30000, 34800, 40000,
                45200, 50400, 56000, 60000, 64000, 68800, 73600, 78400, 84000, 89600, 95200, 100800, 108000, 120000,
                136000, 156000, 180000, 208000, 240000, 276000, 316000, 360000, 440000, 540000, 640000, 760000, 880000,
                1020000, 1160000, 1320000, 1480000, 1680000, 1880000, 1880000, 1880000, 1880000, 1880000, 1880000,
                1880000, 1880000, 1880000, 1880000, 1880000,
            ],
            advanced: [
                60, 180, 360, 600, 1800, 3600, 5400, 7200, 9000, 11100, 13500, 16200, 19200, 22500, 26100, 30000, 33900,
                37800, 42000, 45000, 48000, 51600, 55200, 58800, 63000, 67200, 71400, 75600, 81000, 90000, 102000,
                117000, 135000, 156000, 180000, 207000, 237000, 270000, 330000, 405000, 480000, 570000, 660000, 765000,
                870000, 990000, 1110000, 1260000, 1410000, 1410000, 1410000, 1410000, 1410000, 1410000, 1410000,
                1410000, 1410000, 1410000, 1410000,
            ],
        };

        const cumulativeExp = Object.keys(expCosts).reduce((acc, rarity) => {
            acc[rarity] = expCosts[rarity].reduce((a, c, i) => {
                a.push((a[i - 1] || 0) + c);
                return a;
            }, []);
            return acc;
        ...
        const performCalculation = () => {
            ...
            if (isTomeOnlyMode) {
                const commandersToMax =
                    totalTomeExp > 0 && totalExpToMax > 0 ? Math.floor(totalTomeExp / totalExpToMax) : 0;
    ...
    function initPassportCalculator() {
        ...
        const performCalculation = () => {
            ...
            passportNeededDiv.innerHTML = '';
            costResultDiv.innerHTML = '';
            powerBreakdownDiv.classList.remove('error');
            passportNeededDiv.classList.remove('error');
            ...
            if (hospitalCapacity > 0 && !hospitalTierToggle.disabled) {
                ...
                breakdownLines.push(
                    `Delta w/ Full Hospital: <strong style="color: #ef4444;">-${powerReduction.toLocaleString()}</strong>`,
                );
            }
            ...
            if (hohLinkToggle.checked && hohReturnedPower > 0) {
                ...
                breakdownLines.push(
                    `Delta w/ HoH Return: <strong style="color: #4ade80;">+${hohReturnedPower.toLocaleString()}</strong>`,
                );
            }
            ...
            if (!bracket) {
                powerBreakdownDiv.textContent = 'Could not determine passport bracket.';
                powerBreakdownDiv.classList.add('error');
                return;
            }
            ...
            if (currentBracketIndex > 0) {
                ...
                const passportsSaved =
                    requiredPassports - (isDiscounted ? previousBracket.discount : previousBracket.normal);
                if (powerToDrop > 0 && passportsSaved > 0) {
                    breakdownLines.push(
                        `<span style="font-size: var(--font-size-sm); color: var(--text-secondary);">Drop <strong>${powerToDrop.toLocaleString()}</strong> more power to save <strong>${passportsSaved}</strong> passports.</span>`,
                    );
                }
            }
            ...
            if (migrationDateVal) {
                if (migrationDate < today) {
                    ...
                    const endMonth = migrationDate.getMonth();
                    const endYear = migrationDate.getFullYear();
        ...
        calculatePassportBtn.addEventListener('click', () => {
            ...
            const formElements = [
                powerInput,
                sameKvkCheckbox,
                currentPassportsInput,
                migrationDateInput,
                hospitalCapacityInput,
                hospitalTierToggle,
                hohLinkToggle,
            ];
            setupAutoCalculation(formElements, performCalculation);
    ...
    function initVipCalculator() {
        ...
        const vipLevels = [
            ...
            { level: 4, points: 3500 },
            { level: 5, points: 6000 },
            { level: 6, points: 11500 },
            { level: 7, points: 17500 },
            { level: 8, points: 35000 },
            { level: 9, points: 75000 },
            { level: 10, points: 150000 },
            { level: 11, points: 250000 },
            { level: 12, points: 350000 },
            { level: 13, points: 500000 },
            { level: 14, points: 750000 },
            { level: 15, points: 1000000 },
            { level: 16, points: 1500000 },
            { level: 17, points: 2500000 },
            { level: 18, points: 4000000 },
            { level: 19, points: 6000000 },
            { level: 'SVIP', points: 9000000 },
        ];
        ...
        const performCalculation = () => {
            ...
            const totalPoints = (parseInt(currentVipPointsInput.value.replace(/,/g, ''), 10) || 0) + totalTokenPoints;

            const targetPoints = parseInt(desiredVipLevelSelect.value, 10);
            if (isNaN(targetPoints)) {
                vipResultDiv.textContent = 'Please select a desired VIP level.';
        ...
        const customSelectContainer = document.querySelector(
            '.carousel-slide[data-title="VIP Level"] .custom-select-container',
        );
        if (customSelectContainer && !customSelectContainer.querySelector('.select-selected')) {
            ...
            vipLevels.forEach((vip, index) => {
                ...
                itemDiv.addEventListener('click', function () {
                    ...
                    Array.from(optionsDiv.getElementsByClassName('same-as-selected')).forEach((el) =>
            ...
            function closeAllSelect(elmnt) {
                ...
                for (let i = 0; i < allSelectItems.length; i++) {
                    if (elmnt == null || allSelectItems[i].previousElementSibling != elmnt)
    ...
    function initBuildingCalculator() {
        ...
        const buildingData = {
            castle: {
                ...
                currencyImage: getImagePath('book_of_covenant.webp'),
                costs: [
                    0, 2, 5, 8, 15, 20, 30, 40, 50, 70, 80, 100, 125, 150, 300, 500, 700, 900, 1200, 1500, 1750, 2000,
                    2500, 3000, 5000,
                ],
            },
            watchtower: {
                currencyName: 'Arrows of Resistance',
                currencyImage: getImagePath('arrow_of_resistance.webp'),
                costs: [
                    0, 2, 5, 8, 15, 20, 30, 40, 50, 70, 80, 100, 125, 150, 300, 500, 700, 900, 1200, 1500, 1800, 2000,
                    2500, 3000, 5000,
                ],
            },
            stateForum: {
                currencyName: "Sage's Testimony",
                currencyImage: getImagePath('sages_testimony.webp'),
                costs: [
                    0, 0, 10, 15, 15, 20, 25, 40, 50, 60, 90, 120, 170, 205, 250, 310, 390, 450, 620, 540, 1000, 1400,
                    2400, 4400, 7430,
                ],
            },
        };
        ...
        const performCalculation = () => {
            ...
            if (isNaN(currentLevel) || isNaN(desiredLevel) || currentLevel < 1 || desiredLevel > 25) {
                if (currentLevelInput.value || desiredLevelInput.value) {
    ...
    window.requestAnimationFrame(() => {
        ...
        document.querySelectorAll('.calculator-island input, .calculator-island select').forEach((input) => {
            const eventType =
                input.type === 'radio' || input.type === 'checkbox' || input.tagName === 'SELECT' ? 'change' : 'input';
