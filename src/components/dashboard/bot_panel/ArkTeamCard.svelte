    ...
    <div class="team-body">
        ...
        <div class="signup-container">
            ...
            <div class="add-signup-row">
                <input type="text" placeholder="Manually add player..." bind:value={newSignupName} on:keydown={(e) => e.key === 'Enter' && addManualSignup()} />
                <button on:click={addManualSignup} disabled={!newSignupName} aria-label="Add Player"><i class="fas fa-plus"></i></button>
            </div>
        </div>
    </div>
</div>
...
<style>
    .team-card { background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px; overflow: hidden; display: flex; flex-direction: column; height: 100%; box-shadow: 0 2px 5px rgba(0,0,0,0.1); transition: transform 0.2s; }
    .team-card:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(0,0,0,0.2); border-color: var(--accent-blue-light, #60a5fa); }
    
    .team-header { background: rgba(0,0,0,0.2); padding: 12px 15px; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center; }
    .header-main { display: flex; align-items: center; gap: 10px; flex: 1; }
    .team-badge { background: var(--accent-blue); color: white; padding: 2px 8px; border-radius: 4px; font-size: 0.75rem; font-weight: 700; }
    
    .team-name-input { background: transparent; border: 1px solid transparent; color: var(--text-primary); font-weight: 700; font-size: 1rem; width: 100%; padding: 4px; border-radius: 4px; transition: all 0.2s; }
    .team-name-input:hover { border-color: var(--border-color); background: var(--bg-primary); }
    .team-name-input:focus { outline: none; border-color: var(--accent-blue); background: var(--bg-primary); }
    
    .btn-icon.danger { background: transparent; border: none; color: var(--text-muted); cursor: pointer; transition: color 0.2s; }
    .btn-icon.danger:hover { color: #ef4444; }

    .team-body { padding: 15px; display: flex; flex-direction: column; gap: 15px; flex: 1; }

    .settings-row { display: flex; gap: 10px; }
    .field { flex: 1; display: flex; flex-direction: column; gap: 6px; }
    .field.cap { flex: 0 0 70px; }
    .field label { font-size: 0.75rem; color: var(--text-secondary); font-weight: 600; display: flex; gap: 5px; align-items: center; }
    ...
    .list-header { padding: 8px 12px; background: rgba(0,0,0,0.1); border-bottom: 1px solid var(--border-color); font-size: 0.8rem; font-weight: 600; color: var(--text-secondary); display: flex; justify-content: space-between; align-items: center; }
    
    .count-badge { background: var(--bg-tertiary); padding: 2px 6px; border-radius: 10px; font-size: 0.7rem; }
    .count-badge.full { background: rgba(239, 68, 68, 0.2); color: #ef4444; }
    ...
    .signup-list li:hover { background: var(--bg-tertiary); }
    .signup-list li:last-child { border-bottom: none; }
    .empty-list { font-style: italic; color: var(--text-muted); justify-content: center; padding: 20px; }
    ...
    .add-signup-row { display: flex; border-top: 1px solid var(--border-color); background: rgba(0,0,0,0.05); }
    .add-signup-row input { flex: 1; background: transparent; border: none; padding: 10px 12px; color: var(--text-primary); font-size: 0.85rem; outline: none; }
    .add-signup-row button { background: transparent; border: none; border-left: 1px solid var(--border-color); width: 40px; color: var(--accent-blue); cursor: pointer; transition: background 0.2s; }
    .add-signup-row button:hover { background: rgba(59, 130, 246, 0.1); }
    .add-signup-row button:disabled { color: var(--text-muted); cursor: not-allowed; }
</style>