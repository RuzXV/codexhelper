<script>
    ...
    async function triggerAnimation() {
        ...
        setTimeout(() => { resultAnimationTrigger = false; }, 1200);
    }
    ...
    function calculateAllTroops() {
        ...
        const categories = [
            { key: 't4', baseTime: BASE_TIME_T4, res: RESOURCES.t4, power: POWER_PER_UNIT.t4, mge: MGE_POINTS_PER_UNIT.t4 },
    ...
    function calculateSpeedups() {
        ...
        if (calculatedTotalTroops > 0) {
            ...
            totalPower = (troopBreakdown.t4 * POWER_PER_UNIT.t4) + (troopBreakdown.t5 * POWER_PER_UNIT.t5) + (troopBreakdown.upgrade * POWER_PER_UNIT.upgrade);
            totalMge = (troopBreakdown.t4 * MGE_POINTS_PER_UNIT.t4) + (troopBreakdown.t5 * MGE_POINTS_PER_UNIT.t5) + (troopBreakdown.upgrade * MGE_POINTS_PER_UNIT.upgrade);
            hasResult = true;
            triggerAnimation();
        } else {
            resetResults();
    ...
    function calculateMge() {
        ...
        const avgPoints = (mixRatio.t4 * MGE_POINTS_PER_UNIT.t4) + (mixRatio.t5 * MGE_POINTS_PER_UNIT.t5) + (mixRatio.upgrade * MGE_POINTS_PER_UNIT.upgrade);
    ...
    function handleInput(e, category, type) {
        ...
        troopInputs[category][type] = value ? parseInt(value).toLocaleString('en-US') : '';
...
<div>
    <div class="generator-tabs">
        <button class="generator-tab-btn" class:active={activeTab === 'troops'} on:click={() => switchTab('troops')}>Per Troops</button>
        <button class="generator-tab-btn" class:active={activeTab === 'speedups'} on:click={() => switchTab('speedups')}>Per Speedups</button>
        <button class="generator-tab-btn" class:active={activeTab === 'mge'} on:click={() => switchTab('mge')}>Per MGE Points</button>
    </div>

    <div class="form-group header-group">
        <label for="train-speed">Training Speed (%)</label>
        <div class="tooltip-wrapper">
            <button class="info-btn" on:click|stopPropagation={toggleTooltip} aria-label="Help" type="button"><i class="fas fa-question-circle"></i></button>
            {#if showTooltip}
    ...
    {:else}
        <div class="form-group">
            ...
            <div class="mix-slider-track" bind:this={sliderNode}
                role="slider"
                tabindex="0"
                aria-valuemin="0"
                aria-valuemax="100"
            ...
            <div class="slider-instruction">Drag the sliders on either end towards the middle to adjust your T4/T5/Upgrade split</div>
        </div>
            ...
            {:else if activeTab === 'mge'}
                <div class="form-group fade-in-panel">
                    ...
                    <div class="mge-input-row">
                        <input type="text" class="standalone-input" placeholder="Example: 50,000,000" value={targetMgePoints} on:input={(e) => handleSingleInput(e, (v) => targetMgePoints = v)}>
    ...
    <div class="calc-result" class:result-success={hasResult && resultAnimationTrigger} class:has-value={hasResult}>
        {#if hasResult}
            {#if activeTab === 'troops'}
...
<style>
    ...
    .generator-tab-btn { flex: 1; padding: var(--spacing-3) 4px; cursor: pointer; background: none; border: none; color: var(--text-secondary); font-weight: 600; position: relative; transition: color 0.2s ease; font-size: 0.95rem; white-space: nowrap; }
    ...
    .time-input-group { flex: 1; display: flex; align-items: center; background: var(--bg-primary); border: 1px solid var(--border-hover); border-radius: var(--radius-sm); overflow: hidden; }
    .time-input-group input { flex: 1; border: none; background: transparent; text-align: center; padding: 8px; font-size: 1rem; color: var(--text-primary); min-width: 0; border-radius: var(--radius-sm) 0 0 var(--radius-sm) !important; -webkit-appearance: none; appearance: none; }
    .time-input-group span { padding: 0 10px; background: var(--bg-tertiary); color: var(--text-secondary); font-size: 0.8rem; height: 100%; display: flex; align-items: center; border-left: 1px solid var(--border-hover); }
    ...
    .mix-slider-track { position: relative; height: 24px; background: var(--bg-primary); overflow: visible; cursor: pointer; border: 1px solid var(--border-hover); touch-action: none; user-select: none; border-radius: 2px; }
    .mix-segment { position: absolute; top: 0; bottom: 0; }
    .mix-thumb { position: absolute; top: -6px; bottom: -6px; width: 16px; background: white; border-radius: 8px; transform: translateX(-50%); box-shadow: 0 2px 5px rgba(0,0,0,0.5); border: 2px solid var(--bg-tertiary); z-index: 20; transition: transform 0.1s ease; }
    .mix-thumb:hover { transform: translateX(-50%) scale(1.1); background: var(--accent-blue-light); cursor: grab; }
    .mix-thumb:active { cursor: grabbing; }
    .mix-thumb::after { content: ''; position: absolute; top: -15px; bottom: -15px; left: -15px; right: -15px; }
    .slider-instruction { font-size: 0.75rem; font-style: italic; color: var(--text-secondary); margin-top: 8px; text-align: center; }

    .dynamic-input-container { min-height: 85px; display: flex; flex-direction: column; justify-content: center; }
    .fade-in-panel { animation: fadeInPanel 0.3s ease-out; }
