...
<div class="speedups-hub">
    <div class="hub-tabs">
        <button class="hub-tab-btn" class:active={activeTool === 'training'} on:click={() => (activeTool = 'training')}>
            <img src={images['training_speedup.webp']} alt="" class="tab-icon" />
            <span>Training</span>
        </button>
        <button class="hub-tab-btn" class:active={activeTool === 'healing'} on:click={() => (activeTool = 'healing')}>
            <img
...
<style>
    ...
    .speedups-hub :global(.tooltip-img.square) {
        aspect-ratio: 1/1;
        object-fit: cover;
    }
    .speedups-hub :global(.tooltip-img.wide) {
        height: auto;
    }
    ...
    .speedups-hub :global(.ratio-val) {
        font-weight: 700;
        color: white;
        font-size: 0.9rem;
    }
    .speedups-hub :global(.ratio-lbl) {
        font-size: 0.65rem;
        color: var(--text-secondary);
        margin-top: 2px;
        text-align: center;
    ...
    .speedups-hub :global(.ratio-btn.active .ratio-lbl) {
        color: white;
    }
    ...
    .speedups-hub :global(.troop-grid.t4-group::after) {
        background-image: radial-gradient(circle, #ca62e6 0%, #8113a7 100%);
    }
    .speedups-hub :global(.troop-grid.t5-group::after) {
        background-image: radial-gradient(circle, #f28d00 0%, #d55800 100%);
    }
    .speedups-hub :global(.troop-grid.upgrade-group::after) {
        background: linear-gradient(135deg, #ca62e6 0%, #8113a7 45%, #f28d00 55%, #d55800 100%);
        opacity: 0.25;
    }
    ...
    @keyframes global-glow-border {
        ...
        25% {
            ...
            box-shadow: 0 0 15px 0 var(--accent-green);
        }
        100% {
            border-color: transparent;
